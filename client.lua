...
local function CraftItem(craftedItem, requiredItems, amountToCraft, xpEarned, xpType)
    KGCore.Functions.TriggerCallback('crafting:getPlayerInventory', function(inventory)
        ...
        for _, reqItem in pairs(requiredItems) do
            ...
            if itemAmount < reqItem.amount then
                ...
                KGCore.Functions.Notify(string.format(Lang:t('notifications.notenoughMaterials')) .. amountToCraft .. 'x ' .. KGCore.Shared.Items[craftedItem].label, 'error')
                break
            end
        end
        if hasAllMaterials then
            if Config.EnableSkillCheck then
                local success = exports['kg-minigames']:Skillbar('easy', '12345') -- difficulty and words to enter 
                if success then
                    KGCore.Functions.Progressbar('crafting_item', 'Crafting ' .. KGCore.Shared.Items[craftedItem].label, (math.random(2000, 5000) * amountToCraft), false, true, {
                        ...
                        TriggerServerEvent('kg-crafting:server:receiveItem', craftedItem, requiredItems, amountToCraft, xpEarned, xpType)
                    end)
                else
                    ...
                    TriggerServerEvent('kg-crafting:server:removeMaterials', randomItem.item, randomAmount)
                    KGCore.Functions.Notify('Crafting failed, some materials have been lost!', 'error')
                end
            else
                KGCore.Functions.Progressbar('crafting_item', 'Crafting ' .. KGCore.Shared.Items[craftedItem].label, (math.random(2000, 5000) * amountToCraft), false, true, {
                    disableMovement = true,
            ...
            KGCore.Functions.Notify(string.format(Lang:t('notifications.notenoughMaterials')), 'error')
        end
    end)
end
...
local function CraftAmount(craftedItem, requiredItems, xpGain, xpType)
    ...
    if dialog and tonumber(dialog.amount) then
        ...
        if amount > 0 then
            ...
            KGCore.Functions.Notify(string.format(Lang:t('notifications.invalidAmount')), 'error')
        end
    else
        KGCore.Functions.Notify(string.format(Lang:t('notifications.invalidInput')), 'error')
    end
end
...
local function OpenCraftingMenu(benchType)
    ...
    KGCore.Functions.TriggerCallback('crafting:getPlayerInventory', function(inventory)
        ...
        for _, recipe in pairs(recipes) do
            if currentXP >= recipe.xpRequired then
                ...
                for _, reqItem in pairs(recipe.requiredItems) do
                    ...
                    local itemLabel = KGCore.Shared.Items[reqItem.item].label
                ...
                local menuItem = {
                    header = KGCore.Shared.Items[recipe.item].label,
                    txt = itemsText,
                    icon = Config.ImageBasePath .. KGCore.Shared.Items[recipe.item].image,
                    params = {
                        isAction = true,
                        event = function()
        ...
        exports['kg-menu']:openMenu(menuItems)
    end)
end
...
local function PickupBench(benchType)
    ...
    if DoesEntityExist(entity) then
        DeleteEntity(entity)
        TriggerServerEvent('kg-crafting:server:addCraftingTable', benchType)
        KGCore.Functions.Notify(string.format(Lang:t('notifications.pickupBench')), 'success')
    end
end

-- Events

RegisterNetEvent('kg-crafting:client:useCraftingTable', function(benchType)
    local playerPed = PlayerPedId()
    ...
    TriggerServerEvent('kg-crafting:server:removeCraftingTable', benchType)
    exports['kg-target']:AddTargetEntity(Config[benchType].object, {
        options = {
            {
                icon = 'fas fa-tools',
